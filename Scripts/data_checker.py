...
#Qui saranno espletate le funzioni necessarie alla formattazione corretta del file
...
f = open('../ConfigurationFiles/configuration.txt',"w+")
f.write('')
f.close()
...
for interface in int_list:
	if count_interface!= len(int_list):
		...
		if problem_rilevator == False:
			f = open('../ConfigurationFiles/configuration.txt','a+')
